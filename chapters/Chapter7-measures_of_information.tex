...
\end{example}
We define an ensemble $\mathbf X$ as the tuple of a probability distribution $p_{\mathbf X}$ together with its domain $\mathcal A_X$. 
Generally, we will refer to $\mathcal A_X$ as the sample space of $\mathbf X$ and to its elements as events. %and its elements differently depending on the context. as the sample space and to any non empty subset of it as an event. 
In information theory, $\mathbf X$ typically models an object in a communications setup (see \ref{}), in this context it is common to call $\mathcal A_X$ the alphabet of $\mathbf X$ and refer to its elements as letters. %and its elements differently depending on the context. as the sample space and to any non empty subset of it as an event. 

Note that we can extend the definition of $p_{\mathbf X}$ to any subset $\mathcal{S}\subseteq \mathcal{A}_X$:
\begin{equation}
p_{\mathbf X}(\mathcal{S})=\sum_{x\in \mathcal{S}}p_{\mathbf X}(x)
\end{equation} 
\begin{example}
Let $\mathbf X$ be an ensemble with alphabet $\mathcal A_{mathbf X}=\{1,2,3\}$ and with $p_{\mathbf X}$ the uniform distribution. Then if $\mathcal S=\{1,2\}$, $p(\mathcal S)=1/3+1/3=2/3$ 
\end{example}
...
Given an ensemble $\mathbf X$ and two events $a,b$ we say that they are independent if:
\begin{equation}
p_{\mathbf X}(a \text{ and } b)=p_{\mathbf X}(a)p_{\mathbf X}(b)
\end{equation}
Let $a$ and $b$ be two events with non zero probability. We call $p_{\mathbf X}(a|b)=p_{\mathbf X}(a \text{ and } b)/p_{\mathbf X}(b)$ the conditional probability of $a$ given that $b$ occurs. It follows that if and only if $a$ and $b$ are independent $p_{\mathbf X}(a|b)=p_{\mathbf X}(a)$.

In the following, we will use the explicit notation $p_{\mathbf X},\mathcal A_{\mathbf X}$ for the probability distribution of ensemble $\mathbf X$ and its alphabet whenever confusion can arise but we will drop the subscript whenever possible.
\begin{exercise}
Let $\mathbf X$ be an ensemble modelling two fair coins. Identify two events $a,b$ that are independent and verify that $p_{\mathbf X}(a|b)=p_{\mathbf X}(a)$
\end{exercise}
\begin{solution}
A
\end{solution}
Given two alphabets $\mathcal A_{\mathbf X},\mathcal A_Y$ we can define a joint ensemble on them with sample space or alphabet the direct product: $\mathcal A_{XY} = \mathcal A_X \times \mathcal A_Y$. 
